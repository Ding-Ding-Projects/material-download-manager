...
`clickableControlsContract.test.ts` rejects a generic tab creation path and the
built-artifact smoke activates the tab-strip Add download action, verifies the
form, checks focus return, and proves no duplicate fallback tab appears. Run
