const UI_TEXT = {
  ...
  handoffPending: { en: ["The URL was accepted; the local manager is still checking it.", "The local manager accepted the URL and is still checking it.", "The URL is in the local manager's queue while it puts on its tiny inspector hat.", "The local manager accepted the URL; the slow probe is still tying its shoelaces.", "The URL has checked in, and the local manager is carefully deciding what size of download suitcase it needs."], yue: ["網址已接收，本機程式仲檢查緊。", "本機程式收咗條網址，仲檢查緊。", "網址入咗本機隊伍，檢查員戴緊小帽。", "本機程式收咗條網址，慢速檢查仲喺度綁鞋帶。", "網址已登記，本機程式小心計緊要用幾大個下載行李箱。"] },
