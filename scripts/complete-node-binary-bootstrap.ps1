...
$esbuildPackageJson = Join-Path $designRoot 'node_modules/esbuild/package.json'
...
foreach ($path in @($electronInstall, $esbuildInstall, $esbuildPackageJson)) {
...
$declaredEsbuildVersion = [string]((Get-Content -Raw -LiteralPath $esbuildPackageJson | ConvertFrom-Json).version)
if ([string]::IsNullOrWhiteSpace($declaredEsbuildVersion) -or $esbuildVersion -ne $declaredEsbuildVersion) {
