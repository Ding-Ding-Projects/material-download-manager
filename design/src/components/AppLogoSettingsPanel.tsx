...
export default function AppLogoSettingsPanel({
  ...
  async function chooseLocalImage() {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : ui.text("The local image could not be converted safely.", "本機圖片未能安全轉換。"));
    } finally {
      setBusy(null);
    }
  }
  ...
  async function applyRendering() {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : ui.text("The prior valid logo remains active because conversion did not finish.", "因為轉換未完成，之前有效嘅標誌會保持啟用。"));
    } finally {
      setBusy(null);
    }
  }
  ...
  async function resetLogo() {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : ui.text("The logo reset did not complete.", "標誌重設未完成。"));
    } finally {
      setBusy(null);
    }
  }
