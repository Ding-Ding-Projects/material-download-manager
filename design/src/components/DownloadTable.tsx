  ...
  return (
    ...
    >
      ...
      {menu && primary && menu.mode === "menu" && (
        <ContextMenu x={menu.x} y={menu.y} onClose={closeMenu} label={copy.text("Download actions", "下載項目操作")}>
          <ContextActionList label={copy.text("Download actions", "下載項目操作")} actions={actions} copy={copy} />
        </ContextMenu>
      )}

      {menu && menu.mode === "confirmRemove" && (
        <ContextMenu x={menu.x} y={menu.y} onClose={closeMenu} label={copy.text("Removal choices", "移除選擇")}>
...
function Row({
  ...
  function handleRowKeyDown(event: ReactKeyboardEvent<HTMLTableRowElement>) {
    ...
    if (event.key === "ArrowDown" || event.key === "ArrowUp") {
      const rows = Array.from(document.querySelectorAll<HTMLTableRowElement>("tr[data-download-row]"));
