...
export class ConverterService {
  ...
  async outputPathForJob(id: string): Promise<string | null> {
    this.assertReady();
    const job = await this.readJob(id);
    if (!job || !job.outputAvailable || !job.outputPath) return null;
    if (!await regularFileExists(job.outputPath)) return null;
  ...
  private async executeJob(initial: InternalJob): Promise<void> {
    ...
    try {
      ...
      if (await regularFileExists(outputPath)) throw new Error("The destination appeared while conversion was running; it was not overwritten.");
      await fsp.rename(temporaryOutputPath, outputPath);
      await this.persistJob({
        ...running,
        status: "succeeded",
