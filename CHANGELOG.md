...
- **Released source commits:** [`a8c0178`](https://github.com/Ding-Ding-Projects/material-download-manager/commit/a8c01787481db5c941a11d463b89b7c25d421086) repaired the stuck destructive authorization completion and browser-delivery surfaces; [`ce67440`](https://github.com/Ding-Ding-Projects/material-download-manager/commit/ce674404884986b0b47293deae62ecae9fd781c6) repaired release-draft publication waiting; [`594002b`](https://github.com/Ding-Ding-Projects/material-download-manager/commit/594002bcbe9022632e4399ce294507311a2f1586) now requires a rendered protocol-3 Start-download window before the desktop acknowledges a browser handoff and leaves an unpaired Chrome download untouched. This source shipped in [`v0.1.177`](https://github.com/Ding-Ding-Projects/material-download-manager/releases/tag/v0.1.177).
- **Scope:** a generic unpaired release ZIP leaves Chrome downloads untouched rather than blinking pause/resume; an app-prepared extension pauses only its owned browser item, receives a pending response only after the always-on-top **Start download** window reaches `ready-to-show`, starts the segmented desktop transfer only after the user approves, and cancels the Chrome copy only after its authenticated accepted decision. The separate **Downloading** window remains non-topmost, closing it does not stop the transfer, and app-owned **Download complete** remains always on top.
- **Desktop routing and tray polish:** category roots now come from the platform-known Downloads folder, so redirected user Downloads locations remain authoritative while the existing legacy app-subfolder migration remains file-safe. The tray’s tooltip and restore command now follow the persisted display name, and the startup setting explains that login startup opens in the notification area.
- **Destructive-action recovery:** after both authorization keys and the full slider complete, the gate now calls the real removal path exactly once without waiting for a renderer timer. The dialog therefore closes into the normal result notification instead of presenting a completed authorization state that can appear stuck.
- **Capture boundary:** prior Add, Downloading, and completion captures are historical evidence for the earlier visual surfaces; this source-only protocol refresh has no new local capture claim.
- **Verification boundary:** local tests, typecheck, build, smoke, and capture were deliberately not run for the current source refresh at the user's request. Earlier Electron **149/149**, browser extension **33/33**, documentation **2/2**, and built UI smoke **55/55** are historical evidence only, not a verdict for the current uncommitted source.
