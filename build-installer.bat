...
if "%SILENT_MODE%"=="1" (
  "%PS_EXE%" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%REPOSITORY_ROOT%scripts\build-contract.ps1" -Mode Installer -Silent
) else (
  "%PS_EXE%" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%REPOSITORY_ROOT%scripts\build-contract.ps1" -Mode Installer
)
...
echo Builds and verifies the unsigned Squirrel.Windows installer without publishing it.
echo SILENT=1 or MDM_BUILD_SILENT=1 also enables silent mode.
endlocal & exit /b 0
